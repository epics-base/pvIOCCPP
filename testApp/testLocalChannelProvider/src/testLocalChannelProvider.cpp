...
public:
    ...
    MyRequester()
    : name(String("testLocalChannelProvider")),
...
static const iocshFuncDef testLocalChannelProviderFuncDef = {
    "testLocalChannelProvider", 1, testArgs};
static void testLocalChannelProviderCallFunc(const iocshArgBuf *args)
{
    char *pvName = args[0].sval;
    printf("testLocalChannelProvider pvName %s\n",pvName);
...
static void testLocalChannelProviderRegister(void)
{
    static int firstTime = 1;
    if (firstTime) {
        firstTime = 0;
        iocshRegister(&testLocalChannelProviderFuncDef, testLocalChannelProviderCallFunc);
    }
}
epicsExportRegistrar(testLocalChannelProviderRegister);
